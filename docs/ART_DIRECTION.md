...
- Skin colorways receive distinct code-native markings on the generated sprite path. The Bone Cat renderer key is corrected, and the animated closet preview can display locked combinations without changing the equipped save.
- Skin colorways now use a frame-sized pose-aware palette cache instead of whole-image CSS filters. The classifier recolors only the authored warm-orange fur family, preserves luminance texture, and leaves cream, gold, pink, black, white, and cosmetic pixels untouched; the cache is capped for mobile memory safety.
...
- Hats, glasses, and code-native pattern accents remain separate pose-local canvas layers above the selectively recolored character frame.
